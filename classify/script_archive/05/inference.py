...
if __name__ == '__main__':
    ...
    model_pth = "models/20220608-205056"
    max_length = 110
    ...
    def preprocess_function(examples):
        ...
        """
        result = tokenizer(
            examples["context"],examples["entailment"], truncation=True, padding=False, max_length=max_length)
        return result
    ...
    for text, test_example in test_examples.items():
        ...
        label = list(test_example.label - set([4]))
        labels.append(label[0])
        pred = 0
        for i,(k,v) in enumerate(test_example.pred.items()):
            if i == 4:
                continue
            if v > pred:
                pred = v
