from src.dataset import word_context_entailment_extra_dataset, combine_train_test
...
def get_dataset(train_data_path, test_data_path):
    ...
    # train_dataset = word_context_dataset(train_data_pairs, context_per_text=1)
    # test_dataset = word_context_dataset(test_data_pairs, context_per_text=1)
    # train_dataset = word_context_entailment_dataset(train_data_pairs)
    # test_dataset = word_context_entailment_dataset(test_data_pairs)
    # train_dataset = word_entailment_dataset(train_data_pairs)
    # test_dataset = word_entailment_dataset(test_data_pairs)
    train_dataset = word_context_entailment_extra_dataset(train_data_pairs)
    test_dataset = word_context_entailment_extra_dataset(test_data_pairs)
...
def evaluate(model, dev_dataloader, device):
    ...
    macro_f1 = f1_score(labels, preds, average='binary')
    return macro_f1
...
if __name__ == '__main__':
    ...
    def preprocess_function(examples):
        ...
        """
        result = tokenizer(
            examples["context"],examples["entailment"], truncation=True, max_length=max_length, padding=False)
